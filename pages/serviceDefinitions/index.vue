...
const { 
  ...
  onPageChange,
  onSizeChange,
  onSortChange, getServiceDefinitionById } = useServiceDefinitions();
...
provide('refetchserviceDefinitions', fetchData);
...
<template>
  ...
  <div v-else-if="data && !isError" class="py-5 flex flex-col space-y-10 mx-auto">
    <UiPermissionGuard :permission=PermissionConstants.CREATE_SERVICE_DEFINITION>
      ...
      <template v-slot:toolbar="{ table }">
        <div class="flex items-center justify-between">
          <div class="flex flex-1 items-center space-x-2">
            <UiInput placeholder="Filter by name"
