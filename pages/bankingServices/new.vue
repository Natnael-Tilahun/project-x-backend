...
const { createNewBankingService, isLoading } = useBankingServices();
...
const onSubmit = form.handleSubmit(async (values: any) => {
  try {
    ...
    data.value = await createNewBankingService(values); // Call your API function to fetch profile
    ...
    console.error("Error creating new banking service:", err.message);
    isError.value = true;
  } finally {
...
<template>
  <div class="w-full h-full flex flex-col gap-8">
    <div class="">
      <h1 class="md:text-2xl text-lg font-medium">Create New Banking Service</h1>
      <p class="text-sm text-muted-foreground">
        Create new banking service by including name, service type, permissions, and
        service definition
      </p>
    </div>
